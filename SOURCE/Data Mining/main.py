...
lr = pickle.load(open('model/LogisticRegression.sav', 'rb'))
nb = pickle.load(open('model/NaiveBayes.sav', 'rb'))
p = pickle.load(open('model/Perceptron.sav', 'rb'))
...
if st.button('Get Result'):
    ...
    lr_prediction = classify_order(features_preprocessed, lr)
    nb_prediction = classify_order(features_preprocessed, nb)
    p_prediction = classify_order(features_preprocessed, p)
    st.write('Result of Random Forest:', rf_prediction)
    st.write('Result of Logistic Regression:', lr_prediction)
    st.write('Result of Naive Bayes:', nb_prediction)
    st.write('Result of Perceptron:', p_prediction)
...
if uploaded_file is not None:
    ...
    if not all(col in data.columns for col in required_columns):
        ...
        lr_prediction = lr.predict(preprocessed_data)
        nb_prediction = nb.predict(preprocessed_data)
        p_prediction = p.predict(preprocessed_data)
        ...
        output_data['Predicted_Label_LR'] = ['Profitable' if pred == 1 else 'Unprofitable' for pred in lr_prediction]
        output_data['Predicted_Label_NB'] = ['Profitable' if pred == 1 else 'Unprofitable' for pred in nb_prediction]
        output_data['Predicted_Label_P'] = ['Profitable' if pred == 1 else 'Unprofitable' for pred in p_prediction]
